  ...
  <section class="mb-14">
      <div class="flex flex-col items-center justify-center gap-10 mb-20">
        ...
        <div class="container card p-10 w-[30rem]">
              ...
              {{-- <form action="{{ route('update_Staff', ['facultyData' => $facultyData]) }}" method="post" enctype="multipart/form-data"> --}}
                <form action="{{ route('update_Staff', $facultyData->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('put')
                <div class="mb-3 flex flex-col justify-center items-center">
                    <img id="showPhoto" class="rounded-lg w-40 mb-4" src="{{ asset($facultyData->profileImage) }}" alt="photo"/>
                    <input type="file" name="profileImage" id="profileImage" class="" onchange="previewImage(this);" accept=".jpg, .png, .jpeg">
                </div>
                <div class="mb-3">
                    <label for="name" class="form-label font-sans font-medium capitalize">Name</label>
                    <input type="text" class="form-control border-0 bg-gray-300 rounded" name="name" id="name"  value="{{ $facultyData -> name }}" required>
                </div>
                <div class="mb-3">
                    <label for="role" class="form-label font-sans font-medium capitalize">Role</label>
                    <input type="text" class="form-control border-0 bg-gray-300 rounded" name="role" id="role"  value="{{ $facultyData -> role }}" required>
                </div>
                <div class="gap-4 modal-footer">
                    <a href="faculty_management" type="button" class="btn btn-danger">Cancel</a>
